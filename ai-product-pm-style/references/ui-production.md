...
用于制作、重构或评审 AI/SaaS 产品中的产品界面、页面、组件、交互状态和前端 polish。目标是让 UI 服务用户任务、产品结构和后续迭代，而不是只做表面美化。
...
loading 优先用贴近真实布局的 skeleton；通用 spinner 只用于短暂、局部、无法预测结构的等待。
...
-   tooltip 用于解释不熟悉的图标或短约束；关键说明放在页面流程中。
...
-   移动端 input 字号至少 16px，避免 iOS Safari 自动放大。
-   hover 样式限定到支持 hover 的设备，避免触屏 sticky hover。
-   固定在底部或边缘的移动端 UI 使用 safe-area inset。
-   动效尊重 `prefers-reduced-motion`。

## 视觉 polish

-   卡片、面板、工具栏和列表项使用稳定尺寸，hover、loading、错误状态不造成布局跳动。
-   嵌套圆角保持视觉一致：外层圆角约等于内层圆角加间距。
-   图标按钮做光学校正，图标侧 padding 可以比文字侧少 2-4px。
-   文本容器限制宽度，正文控制在易读行长内；dashboard 内容可以更宽。
-   交互控件之间填满可点击区域，减少列表、菜单、toast 中的不可点击空隙。
-   模态、抽屉、菜单关闭后焦点返回触发元素。
...
-   键盘可操作主要控件和 modal。
-   图标按钮有可访问名称。
-   触屏没有 sticky hover、输入自动放大或底部遮挡。
-   视觉 polish 没有明显对齐、圆角、间距、阴影、滚动跳动问题。
-   使用浏览器截图或手动流程验证关键页面，而不是只看代码 diff。
